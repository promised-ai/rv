...
use nalgebra::base::{EuclideanNorm, Norm};
use nalgebra::{ComplexField, DMatrix, DVector, Dim, Matrix};
use num::Zero;
use std::f64;
use std::fmt;
...
#[inline]
fn e2_norm<N, R1, C1, S1, R2, C2, S2>(
    m1: &Matrix<N, R1, C1, S1>,
    m2: &Matrix<N, R2, C2, S2>,
    scale: N,
) -> N::RealField
...
impl Kernel for RBFKernel {
    ...
    fn consume_parameters(params: &[f64]) -> (Self, &[f64]) {
        assert!(params.len() >= 1, "RBFKernel requires one parameters");
...
impl Kernel for WhiteKernel {
    ...
    fn consume_parameters(params: &[f64]) -> (Self, &[f64]) {
        assert!(params.len() >= 1, "WhiteKernel requires one parameters");
        let (cur, next) = params.split_at(1);
        let ck = Self::from_parameters(cur);
        (ck, next)
    }
